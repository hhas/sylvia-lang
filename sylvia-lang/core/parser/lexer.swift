...
// TO DO: eliminate empty `()` from argument-less commands (users can still get the handler object using `foo as handler`); this avoids awkward syntactic distinction between a [read-only] ivar vs property (e.g. `foo of bar` can be either raw var or a custom handler that returns a generated result); Q. how will writable properties work? (ignoring syntactic sugar, if it distills to `set (#NAME, VALUE) of TARGET` then presumably the default `set` implementation could be overridden, though it'd be good if users didn't have to write this themselves; OTOH, a global `set (NAME of TARGET, VALUE)` wouldn't work the same unless it simply delegates to the above); another option might be to adopt a standard naming convention `get_NAME ()`/`set_NAME (to: VALUE)`


// TO DO: how practical to allow labeled arguments to appear unparenthesized, e.g. `some_command ARG1 some_label: ARG2 another_label: ARG3`? this'd get us close to AS readability, caveat that complex/ambiguous args would require parenthesizing (more akin to Logo syntax when calling vararg procedures); this syntax would depend on pairs always appearing as complete expressions everywhere else (e.g. record fields, local assignments), so that the only time a pair would trail an existing expression would be when that expression is a command name (with, optionally, a single direct argument) (this might get cranky if single argument is parenthesized as it can't tell if parens there are an elective group around argument expression or if they denote the start and end of command's arguments)


/* TO DO:
 
     text literals should use `««EXPR»»` "tags", not backslash escapes, for character substitution and interpolation; e.g. `"Hello,««name»»!"` is arguably easier to understand than `"Hello,\(name)!"`; in turn, character and unicode escapes would be "foo««linebreak»»bar««0u12AB»»" rather than "foo\nbar\u12AB". Note that `linebreak`, `tab`, etc would need to be defined as non-maskable identifiers/atom operators in stdlib, and `case digitCharacters` in Lexer.tokenize() (or Lexer.readNumber?) will have to recognize `0uCODE…` syntax (this should use UTF-8 notation, so multiple codepoints will need written as `0uAAAA 0uBBBB 0uCCCC` and combined into one Text token by lexer, with formatting annotation). Q. what limits should be placed on interpolated EXPR when interpolating literal strings, e.g. by evaluating it in restricted Environment? (e.g. Side-effects really should be disallowed. One option would be to whitelist stdlib handlers as being safe and relevant for this task; alternatively, allow any stdlib handler but error if any try to perform side effects.) It's probably best to have a single set of rules/limitations for both literal interpolation and text templating (where the format string is supplied at runtime) to avoid confusing users; the only difference would be in how custom identifiers and handlers are made available in latter case. (Whereas string literals can be automatically allowed to bind any symbol in their lexical scope, a runtime templating function would need all custom resources explicitly passed in, e.g. assuming an Environment instance is used to represent a custom object, it would need read locks to permit client code to access its own handlers but not those of its parent scopes, while still allowing those handlers to access parent scopes themselves, plus write locks so that client code can't change anything.)
