...
func stdlib_loadCoercions(env: Env) throws {
    ...
    try env.add(AsDefault(asAnythingOrNothing, noValue)) // note: AsDefault requires constraint args (type and defaultValue) to instantiate; native language will call() it to create new instances with appropriate constraints
}
...
func stdlib_loadConstants(env: Env) throws {
    try env.set("nothing", to: noValue) // TO DO: should `nothing` be both Value and TYPE? e.g. `foo() returning errorOr(nothing)`? (note: primitive handlers use `asNoResult` as signature's return type to indicate no return value, in which case bridge code generator changes return statement to `return noValue`)
