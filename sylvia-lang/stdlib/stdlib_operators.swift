...
let stdlib_operators: [OperatorDefinition] = [
    ...
    ("+",   490, .prefix(parseNumericSignOperator), ["positive"], command: "positive"),
    ("-",   490, .prefix(parseNumericSignOperator), ["negative"], command: "negative"),
    
    ("×",   480, .infix(parseInfixOperator),      ["*", "multiply"], command: nil),
    ("÷",   480, .infix(parseInfixOperator),      ["/", "divide"], command: nil),
    
    ("div", 480, .infix(parseInfixOperator),      ["//", "integerDivision", "divideKeepingWhole"], command: nil), // "//" = Python div operator
    ("mod", 480, .infix(parseInfixOperator),      ["%", "modulus", "divideKeepingFraction"], command: nil), // note: if using `%` as a quantity unit (e.g. "100%"), need to make sure it won't be confused for modulus operator (one option is to define it as a postfix operator, which is one way to implement general quantity support [although this approach could get hairy when defining lots of postfix ops with common names like `meters`, `mm`; as a syntactic rule of thumb, it's safer to use coercions to convert to/from/between quantities, as that only has to define one type handler for each unit category: `length(units,min,max)`, `weight(…)`, `volume(…)`, etc])
    
    ("+",   470, .infix(parseInfixOperator),      ["add"], command: nil),
    ("-",   470, .infix(parseInfixOperator),      ["subtract"], command: nil),
    
    // math comparison
    ("<",  400, .infix(parseInfixOperator), ["isLessThan"], command: nil),
    ("<=", 400, .infix(parseInfixOperator), ["isLessThanOrEqualTo", "≤"], command: nil),
    ("==", 400, .infix(parseInfixOperator), ["isEqualTo"], command: nil),    // TO DO: for this exercise we'll stick to traditional C/Python-style syntax, but need to explore using `=` for equality tests (c.f. AppleScript)
    ("!=", 400, .infix(parseInfixOperator), ["isNotEqualTo", "≠"], command: nil),
    (">",  400, .infix(parseInfixOperator), ["isGreaterThan"], command: nil),
    (">=", 400, .infix(parseInfixOperator), ["isGreaterThanOrEqualTo", "≥"], command: nil),
    ...
    // identity comparison
    ("isObject",    400, .infix(parseInfixOperator), [], command: nil), // compare object IDs
    ...
    ("&", 450, .infix(parseInfixOperator), ["concatenatedWith"], command: nil), // TO DO: should this accept optional `as` clause for specifying which type to force both operands to before joining (e.g. `A & B as list(text)`); and, if so, how should constraints be applied? (ditto for text/list comparison operators)
    ...
    // define handlers
    ("to",   0, .prefix(parseHandlerConstructorOperator), ["defineCommandHandler"], command: nil), // TO DO: should aliases redeclare canonical name?
    ("when", 0, .prefix(parseHandlerConstructorOperator), ["defineEventHandler"], command: nil),
    
    // flow control
    ("if",      0, .prefix(parseExpressionAndBlockOperator), [], command: nil),
    ("repeat",  0, .prefix(parseExpressionAndBlockOperator), [], command: nil),
    ("while",   0, .prefix(parseExpressionAndBlockOperator), [], command: nil),
