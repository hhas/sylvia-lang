...
//    ("isSameObjectAs",  400, .infix(parseInfixOperator), [], nil), // compare object IDs
    ...
    ("&", 450, .infix(parseInfixOperator), [], "joinValues"), // TO DO: should this accept optional `as` clause for specifying which type to force both operands to before joining (e.g. `A & B as list(text)`); and, if so, how should constraints be applied? (ditto for text/list comparison operators)
